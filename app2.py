...
app.config['MYSQL_PASSWORD'] = 'Julius@5544'
...
def predict_malnutrition():
    try:
        ...
        # Get recommendation based on predicted status
        recommendation = get_recommendation(predicted_status)

        return render_template('dashboard.html', prediction=predicted_status, recommendation=recommendation)
    except ValueError:
        return "Please enter valid numerical values for age, weight, and height."

def get_recommendation(status, recommendation_type='recommendation'):
    recommendations = {
        'Underweight': {
    ...
    return recommendations.get(status, {}).get(recommendation_type, '')
